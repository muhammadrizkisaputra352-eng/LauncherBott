...
`.trim()
    try {
               ...
               if (global.db.data.users[m.sender].naga == 1) return conn.reply(m.chat, 'Kamu sudah memiliki pet ini', m)
                ...
                case 'foodpet':
                    ...
                    case 'potion':
                        ...
                        break
                    case 'arrow':
                            if (global.db.data.users[m.sender].money >= arrow * count) {
                                global.db.data.users[m.sender].money -= arrow * count
                                global.db.data.users[m.sender].arrow += count * 1
                                conn.reply(m.chat, `✔️ Sukses Membeli ${count} Arrow 🏹️ Dengan Harga ${arrow * count} money 💹`, m)
                            } else conn.reply(m.chat, `Uang Anda Tidak Cukup Untuk Membeli ${count} Arrow Dengan Harga ${arrow * count} Money `,)
                        break
                    case `gold`:
                    ...
                    case 'armor':
                     ...
                     case 'fishingrod':
                        ...
                        break
                    default:
                        return conn.sendThumb(m.chat, Kchat, thumbnail, m)
                }
                break
            case 'sell': 
                switch (_type) {
                    case 'potion':
                        ...
                        break
                    case 'arrow':
                        if (global.db.data.users[m.sender].arrow >= count * 1) {
                            global.db.data.users[m.sender].money += Sarrow * count
                            global.db.data.users[m.sender].arrow -= count * 1
                            conn.reply(m.chat, `✔️ Sukses Menjual ${count} Arrow 🏹️ Dengan Harga ${Sarrow * count} Money 💹 `.trim(), m)
                        } else conn.reply(m.chat, `🏹️ Arrow Kamu Tidak Cukup`.trim(), m)
                        break
                    case 'gold':
            ...
            switch (type) {
                ...
                case 'arrow':
                        if (global.db.data.users[m.sender].money >= arrow * count) {
                            global.db.data.users[m.sender].money -= arrow * count
                            global.db.data.users[m.sender].arrow += count * 1
                            conn.reply(m.chat, `Sukses membeli ${count} Arrow Dengan Harga ${arrow * count} Money`, m)
                        } else conn.reply(m.chat, `Uang Anda Tidak Cukup Untuk Membeli ${count} Potion Dengan Harga ${arrow * count} Money`,m)
            ...
            switch (type) {
                ...
                case 'arrow':
                    if (global.db.data.users[m.sender].arrow >= count * 1) {
                        global.db.data.users[m.sender].money += Sarrow * count
                        global.db.data.users[m.sender].arrow -= count * 1
                        conn.reply(m.chat, `Sukses Menjual ${count} Arrow Dengan Harga ${Sarrow * count} Money`.trim(), m)
                    } else conn.reply(m.chat, `Potion Kamu Tidak Cukup `.trim(), m)
                    break
                case 'common':
                    if (global.db.data.users[m.sender].common >= count * 1) {
        ...
        conn.sendThumb(m.chat, Kchat, thumbnail, m)
        console.log(e)
        if (DevMode) {
