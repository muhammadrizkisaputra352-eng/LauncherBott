let handler = async (m, { conn, args, usedPrefix, DevMode, command}) => {
    if (args.length < 3) {
    let ipahy = `Contoh: *${usedPrefix}${command} money 100 @tag*\n\n*List Yang Bisa Di Transfer :*\n•💵 Money\n•💳 Tabungan\n•🥤 Potion\n•🗑️ Sampah\n•💎 Diamond\n•📦 Common\n•📦 Uncommon\n•👑 Mythic\n•💎 Legendary\n•🕸️ String\n🪵 Kayu\n•🪨 Batu\n•⚙️ Iron\n•🪨 Coal\n•🖍️ Peluru`
        return conn.reply(m.chat, ipahy, m)
    } else try {
    let ipah = `Contoh: *${usedPrefix}${command} money 100 @tag*\n\n*List Yang Bisa Di Transfer :*\n•💵 Money\n•💳 Tabungan\n•🥤 Potion\n•🗑️ Sampah\n•💎 Diamond\n•📦 Common\n•📦 Uncommon\n•👑 Mythic\n•💎 Legendary\n•🕸️ String\n🪵 Kayu\n•🪨 Batu\n•⚙️ Iron\n•🪨 Coal\n•🖍️ Peluru`
            ...
            case 'batu':
                ...
                if (global.db.data.users[m.sender].coal >= count * 1) {
                    try {
                        ...
                        global.db.data.users[who].coal += count * 1
                        conn.reply(m.chat, `Berhasil Mentransfer coal Sebesar ${count}`.trim(), m)
                    } catch (e) {
                        global.db.data.users[m.sender].coal += count * 1
                        m.reply('Gagal Menstransfer')
                        console.log(e)
                ...
                } else conn.reply(m.chat, `coal Kamu Tidak Mencukupi Untuk Mentransfer coal Sebesar ${count}`.trim(), m)
                case 'peluru':
                if (global.db.data.users[m.sender].peluru >= count * 1) {
                    try {
                        global.db.data.users[m.sender].peluru -= count * 1
                        global.db.data.users[who].peluru += count * 1
                        conn.reply(m.chat, `Berhasil Mentransfer peluru Sebesar ${count}`.trim(), m)
                    } catch (e) {
                        global.db.data.users[m.sender].peluru += count * 1
                        m.reply('Gagal Menstransfer')
                        console.log(e)
                ...
                } else conn.reply(m.chat, `peluru Kamu Tidak Mencukupi Untuk Mentransfer peluru Sebesar ${count}`.trim(), m)
                break
            case 'kayu':
